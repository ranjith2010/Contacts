...
- (void)storeUser:(CUser *)user :(storeCDUserComletionBlock)block {
    CDUser *userCoreDataModel = [NSEntityDescription insertNewObjectForEntityForName:@"CDUser" inManagedObjectContext:moc];
    [userCoreDataModel setValue:user.username forKey:kServerUserName];
    [userCoreDataModel setValue:user.email forKey:kServerEmailAttr];
    [userCoreDataModel setValue:user.password forKey:kServerPassword];
    NSError *error;
    BOOL result = [self saveCoreDataStateWithError:&error];
    block(result,error);
}
